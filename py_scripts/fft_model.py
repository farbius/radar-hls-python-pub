...
def main():
    ...
    py_param    = np.loadtxt(os.getcwd() + "/sim_files/fftPy_param.txt", dtype=float)
    Npoints     =   float(py_param[0])
    NoiseFloor  =   float(py_param[1])
    SNR         =   float(py_param[2])
    ...
    plt.legend()
    plt.title("FFT noise floor {:3.2f} dB, input noise floor is {:3.2f} dB, input SNR = {:3.2f} dB".format(10*np.log10(Np), NoiseFloor, SNR), fontweight="bold", fontsize=14)
    plt.xlabel('bin')
    plt.ylabel('power, dB')
    plt.grid()
    ...
    plt.subplot(212)
    plt.plot(np.abs(np.real(py_cmpx)) - np.abs(np.real(hw_cmpx)), '.-r', label='real')
    plt.plot(np.abs(np.imag(py_cmpx)) - np.abs(np.imag(hw_cmpx)), '.-b', label='imag')
