...
def cfar_1d(s, GrdHalf, RefHalf, T, k_th, Type):
    ...
    for idx in range(N):
        if(idx >= RefHalf and idx < (N - RefHalf)):
            for t in range(RefHalf):
                ref_win[t]          = s[idx - t - 1]
                ref_win[t + RefHalf]= s[idx + t - 0]
            ref_win 	            = sort_insertion(ref_win)
            Z 			            = T*ref_win[k_th]
            s_cfar[idx]             = Z
            t_cfar[idx]             = comp_geb(Z, s[idx])
...
def main():
    ...
    plt.figure(1)
    plt.suptitle('OS CFAR, Pfa is {}, window size is {}'.format(PFA, RefHalf*2), fontsize=14, fontweight='bold')
    ...
    plt.legend()
    plt.title("Python float point implementation, {} targets detected".format(np.size(indx)))
    plt.xlabel('bins')
    plt.ylabel('level, dB')
    plt.ylim([-30, 10])
    plt.grid()
    ...
    plt.plot(10*np.log10(np.floor(z0_cfarOS / 2**10)), 'x-b', label="Python OS CFAR Threshold")
    plt.title("HLS Simulation Output, {} targets detected".format(nt))
    ...
    # Error calculation 
    Error = np.nansum(np.abs(np.floor(z0_cfarOS / 2**10) - hls_out))
