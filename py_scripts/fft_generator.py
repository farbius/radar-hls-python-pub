...
parser.add_argument('Noise_dB'  ,default=None, type=int)
args = parser.parse_args()

# example
# python data_generator.py 1024 3 30 -50
...
def main():
    ...
    xu       = np.zeros(Npoints, dtype=complex)
    for k in range(Nsignals):
        nbin = Npoints // 8 * (k + 1)
        x   += 10**((SNR_q + SNR_dB - 10*k)/20) * np.exp(2*1j*np.pi*nbin*np.arange(Npoints)/Npoints) + 10**(SNR_q/20)*np.random.randn(Npoints) # 
    
    for k in range(Npoints):
        if(np.real(x[k])*(Ampl) > Ampl):
            print("Hardware Overflow, please correct Dynamic Range (Noise_dB + SNR_dB) in 0 ... - 92 dB ")
            exit()
            
        if(np.imag(x[k])*(Ampl) > Ampl):
            print("Hardware Overflow, please correct Dynamic Range (Noise_dB + SNR_dB) in 0 ... - 92 dB ")
            exit()
    ...
    np.savetxt(curr_path + '/sim_files/fftPy_param.txt', py_param,fmt='%f')
